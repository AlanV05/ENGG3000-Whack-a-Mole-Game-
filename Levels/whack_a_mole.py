...
# --- Level / Big Mole config -------------------------------------------
# Manual level advance (press N) rather than time or score gated, since
# this is mainly used to demo the three modes rather than a graded climb.
BIG_MOLE_COMBO_THRESHOLD = 5     # combo streak needed to trigger a Big Mole
BIG_MOLE_BONUS_POINTS = BIG_MOLE_COMBO_THRESHOLD  # "5 combo -> worth 5"
BIG_MOLE_TIMEOUT_SECONDS = 0.9   # shorter window than a normal mole - the
                                 # challenge is timing it, not spotting it
BIG_MOLE_SCALE = 1.7            # how much bigger it is than a normal mole
MAX_LEVEL = 3

LEVEL_CONFIGS = {
    1: {
        "label": "Level 1 - Warm Up",
        "timeout": MOLE_TIMEOUT_SECONDS,
        "dual_chance": 0.0,
        "big_mole_enabled": False,
    },
    2: {
        "label": "Level 2 - Combo Mode",
        "timeout": MOLE_TIMEOUT_SECONDS,
        "dual_chance": 0.0,
        "big_mole_enabled": True,
    },
    3: {
        "label": "Level 3 - Overdrive",
        "timeout": 1.1,
        "dual_chance": 0.35,
        "big_mole_enabled": True,
    },
...
COLOR_BIG_MOLE = (216, 155, 56)
COLOR_BIG_MOLE_LIGHT = (255, 210, 130)
COLOR_BIG_MOLE_DARK = (160, 104, 28)
...
class StarBurst:
    ...
    def __init__(self, center, scale=1.0):
        ...
        self.scale = scale
        self.star_count = StarBurst.STAR_COUNT + (2 if scale > 1.2 else 0)
        self.orbit_radius = StarBurst.ORBIT_RADIUS * scale
    ...
    def draw(self, surface, now):
        ...
        for i in range(self.STAR_COUNT):
            ...
            size = (8 + 2 * math.sin(rad * 2)) * self.scale
...
class Mole:
    ...
    BIG_MISS_LINES = ["Ha! Big one got away!", "So close, tiny!", "Better luck next combo!"]

    def __init__(self, timeout_seconds=None, is_big=False, position=None):
        self.position = position if position is not None else random.choice(HOLE_POSITIONS)
        self.spawn_time = pygame.time.get_ticks()
        self.state = Mole.SPAWNING
        self.state_time = self.spawn_time
        self.blink_seed = random.uniform(0, 1000)
        self.is_big = is_big
        self.timeout_seconds = timeout_seconds if timeout_seconds is not None else MOLE_TIMEOUT_SECONDS
        self.radius_scale = BIG_MOLE_SCALE if is_big else 1.0
        self.miss_line = random.choice(Mole.BIG_MISS_LINES if is_big else Mole.MISS_LINES)
    ...
    def update(self, now):
        if self.state == Mole.SPAWNING and (now - self.state_time) >= MOLE_SPAWN_MS:
            ...
            self.state_time = now
        elif self.state == Mole.IDLE and self._elapsed_total(now) >= self.timeout_seconds:
    ...
    def is_expired(self):
        return self._elapsed_total(pygame.time.get_ticks()) >= self.timeout_seconds
    ...
    def contains(self, pos):
        ...
        return (dx ** 2 + dy ** 2) ** 0.5 <= MOLE_RADIUS * self.radius_scale

    def register_hit(self, now):
        self.state = Mole.HIT
        self.state_time = now
        head_point = (self.position[0], self.position[1] - 10 - MOLE_RADIUS * self.radius_scale - 10)
        self.star_burst = StarBurst(head_point, scale=self.radius_scale)
    ...
    def draw(self, surface, now):
        ...
        if self.is_big:
            color, light, dark = COLOR_BIG_MOLE, COLOR_BIG_MOLE_LIGHT, COLOR_BIG_MOLE_DARK
        else:
            color, light, dark = COLOR_MOLE, COLOR_MOLE_LIGHT, COLOR_MOLE_DARK
        face_mode = "happy"
        shake_x = 0
        ...
        body_y = base_y - int(15 * rise * self.radius_scale) - int(4 * (1 - rise) * self.radius_scale)
        radius_x = MOLE_RADIUS * self.radius_scale * squash_x
        radius_y = MOLE_RADIUS * self.radius_scale * squash_y
        cx = x + shake_x
        ...
        if rise > 0.4:
            ...
            paw_w = 15 * self.radius_scale
            paw_h = 13 * self.radius_scale
            for side in (-1, 1):
                paw_cx = cx + side * radius_x * 0.72
                paw_points = [
                    (paw_cx + nx * paw_w * squash_x, paw_y + ny * paw_h * squash_y)
        ...
        self._draw_face(surface, cx, body_y, radius_x, radius_y, face_mode, now, self.radius_scale)
    ...
    def _draw_face(self, surface, cx, body_y, radius_x, radius_y, mode, now, scale=1.0):
        s = scale
        if mode == "happy":
            pygame.draw.circle(surface, (60, 40, 30), (int(cx), int(body_y + 12 * s)), max(3, int(6 * s)))
            blink = (math.sin((now + self.blink_seed) / 900.0) > 0.94)
            if blink:
                pygame.draw.line(surface, (30, 20, 15), (cx - 26 * s, body_y - 12 * s), (cx - 12 * s, body_y - 12 * s), 2)
                pygame.draw.line(surface, (30, 20, 15), (cx + 12 * s, body_y - 12 * s), (cx + 26 * s, body_y - 12 * s), 2)
            else:
                pygame.draw.arc(surface, (30, 20, 15), (cx - 24 * s, body_y - 18 * s, 16 * s, 12 * s), 3.6, 6.0, 2)
                pygame.draw.arc(surface, (30, 20, 15), (cx + 8 * s, body_y - 18 * s, 16 * s, 12 * s), 3.6, 6.0, 2)
            for side in (-1, 1):
                for dy in (-4, 2, 8):
                    pygame.draw.line(
                        surface, (40, 30, 20),
                        (cx + side * 22 * s, body_y + dy * s),
                        (cx + side * 40 * s, body_y + (dy - 2) * s), 1
                    )
            ...
            for side in (-1, 1):
                ex = cx + side * 16 * s
                ey = body_y - 12 * s
                pygame.draw.line(surface, (60, 20, 20), (ex - 5 * s, ey - 5 * s), (ex + 5 * s, ey + 5 * s), 2)
                pygame.draw.line(surface, (60, 20, 20), (ex - 5 * s, ey + 5 * s), (ex + 5 * s, ey - 5 * s), 2)
            pygame.draw.arc(surface, (60, 20, 20), (cx - 10 * s, body_y + 6 * s, 20 * s, 12 * s), 3.4, 6.0, 2)
            ...
            for side in (-1, 1):
                ex = cx + side * 15 * s
                ey = body_y - 14 * s
                pygame.draw.circle(surface, (255, 255, 255), (int(ex), int(ey)), max(3, int(7 * s)))
                pygame.draw.circle(surface, (25, 20, 15), (int(ex + side * 2 * s), int(ey + 1 * s)), max(2, int(4 * s)))
            # smug raised eyebrows
            pygame.draw.line(surface, (30, 20, 15), (cx - 24 * s, body_y - 24 * s), (cx - 8 * s, body_y - 27 * s), 2)
            pygame.draw.line(surface, (30, 20, 15), (cx + 8 * s, body_y - 27 * s), (cx + 24 * s, body_y - 24 * s), 2)
            # nose
            pygame.draw.circle(surface, (60, 40, 30), (int(cx), int(body_y + 10 * s)), max(3, int(6 * s)))
            # tongue sticking out, wagging a little
            wag = math.sin(now / 60.0) * 3
            tongue_rect = (cx - 8 * s + wag, body_y + 16 * s, 16 * s, 14 * s)
...
def draw_hud(surface, font, small_font, score, time_remaining, combo, level):
    ...
    if combo >= 2:
        ...
        next_label = " (max level)" if level >= MAX_LEVEL else " - N for next level"
        instruction_text = small_font.render(f"Level {level}/{MAX_LEVEL}{next_label}", True, COLOR_TEXT)
...
def draw_level_banner(surface, now, banner_until, text, font_big, life_ms=1800):
    """Big centred callout shown briefly right after advancing a level."""
    remaining = banner_until - now
    if remaining <= 0:
        return
    t = clamp(1 - remaining / life_ms)
    fade_in = clamp(t * 6)
    fade_out = clamp((life_ms - remaining) / 250) if remaining < 250 else 1.0
    alpha = int(255 * min(fade_in, fade_out))

    label = font_big.render(text, True, COLOR_TITLE)
    panel_w, panel_h = label.get_width() + 70, 70
    panel = pygame.Rect(0, 0, panel_w, panel_h)
    panel.center = (SCREEN_WIDTH // 2, 190)

    panel_surf = pygame.Surface((panel_w, panel_h), pygame.SRCALPHA)
    pygame.draw.rect(panel_surf, (255, 253, 248, min(235, alpha)), (0, 0, panel_w, panel_h), border_radius=18)
    pygame.draw.rect(panel_surf, (*COLOR_STAR_DARK, alpha), (0, 0, panel_w, panel_h), width=3, border_radius=18)
    surface.blit(panel_surf, panel.topleft)

    text_surf = label.copy()
    text_surf.set_alpha(alpha)
    surface.blit(text_surf, text_surf.get_rect(center=panel.center))


def draw_big_mole_warning(surface, now, warn_until, font, life_ms=1200):
    """Quick telegraph banner right before a Big Mole spawns."""
    remaining = warn_until - now
    if remaining <= 0:
        return
    pulse = 0.6 + 0.4 * math.sin(now / 90.0)
    alpha = int(255 * pulse * clamp(remaining / 300) if remaining < 300 else 255 * pulse)
    label = font.render("BIG MOLE INCOMING!", True, COLOR_BIG_MOLE_DARK)
    text_surf = label.copy()
    text_surf.set_alpha(int(clamp(alpha / 255) * 255))
    rect = text_surf.get_rect(center=(SCREEN_WIDTH // 2, GRID_MARGIN_Y - 25))
...
def run_game():
    ...
    def new_game_state():
        return {
            ...
            "start_ticks": None,
            "moles": [],
            "pending_big_mole": False,
            "level_banner_until": 0,
            "big_mole_warn_until": 0,
    ...
    def spawn_wave():
        """Fills the field for the current level: usually one mole, but a
        pending Big Mole always spawns alone, and Level 3 sometimes drops
        two normal moles into different holes at once."""
        level_cfg = LEVEL_CONFIGS[state["level"]]
        if state["pending_big_mole"]:
            pos = random.choice(HOLE_POSITIONS)
            state["moles"].append(Mole(timeout_seconds=BIG_MOLE_TIMEOUT_SECONDS, is_big=True, position=pos))
            state["pending_big_mole"] = False
            return
        count = 1
        if level_cfg["dual_chance"] > 0 and random.random() < level_cfg["dual_chance"]:
            count = 2
        for pos in random.sample(HOLE_POSITIONS, count):
            state["moles"].append(Mole(timeout_seconds=level_cfg["timeout"], position=pos))
    ...
    def advance_level():
        ...
        state["moles"] = []
        spawn_wave()
        state["level_banner_until"] = pygame.time.get_ticks() + 1800
    ...
    while running:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_ESCAPE:
                    ...
                    start_round()
                elif event.key == pygame.K_n and state["stage"] == STAGE_PLAYING:
                    advance_level()
                ...
                if is_double_click:
                    ...
                    hit_mole = next((m for m in state["moles"] if m.contains(click_pos)), None)
                    if hit_mole is not None:
                        hit_mole.register_hit(now)
                        level_cfg = LEVEL_CONFIGS[state["level"]]
                        if hit_mole.is_big:
                            gained = BIG_MOLE_BONUS_POINTS
                            state["combo"] = 0
                            popup_text, popup_color = f"BIG! +{gained}", COLOR_STAR
                            state["shake_strength"] = 9
                        else:
                            state["combo"] += 1
                            gained = 1
                            popup_text, popup_color = f"+{gained}", COLOR_POPUP
                            state["shake_strength"] = 5
                            if (level_cfg["big_mole_enabled"]
                                    and state["combo"] >= BIG_MOLE_COMBO_THRESHOLD
                                    and not state["pending_big_mole"]):
                                state["pending_big_mole"] = True
                                state["combo"] = 0
                                state["big_mole_warn_until"] = now + 900
                        state["score"] += gained
                        state["popups"].append(FloatingText(hit_mole.position, popup_text, popup_color, font_popup))
                        state["dust_puffs"].append(DustPuff(hit_mole.position))
                        state["shake_until"] = now + 130
        ...
        if state["stage"] == STAGE_TITLE:
            ...
            if time_remaining <= 0:
                ...
                for m in state["moles"]:
                    old_state = m.state
                    m.update(now)
                    if old_state != Mole.MISSED and m.state == Mole.MISSED:
                        state["combo"] = 0  # any escaped mole breaks the combo
                state["moles"] = [m for m in state["moles"] if not m.should_remove()]
                if not state["moles"] and not state["big_mole_warn_until"] > now:
                    spawn_wave()
            ...
            draw_hud(render_target, font, font_small, state["score"], time_remaining,
                     state["combo"], state["level"])
            draw_level_banner(render_target, now, state["level_banner_until"],
                               LEVEL_CONFIGS[state["level"]]["label"], font_big)
            draw_big_mole_warning(render_target, now, state["big_mole_warn_until"], font)
